...
export default function Blog({
    ...
    async function createBlog(ev) {
        ...
        console.log('Submitting blog data:', data); // Debugging

        try {
            if (_id) {
                const response = await axios.put('/api/blogs', { ...data, _id });
        ...
        <>
            <form className='addWebsiteform' onSubmit={createBlog}>
                ...
                <div className="w-100 flex flex-col flex-left mb-2">
                    ...
                    <select
                        onChange={(ev) => setBlogcategory(Array.from(ev.target.selectedOptions, option => option.value))}
                        name='category'
                        id='category'
                        value={blogcategory}
                        multiple
                ...
                <div className='description w-100 flex flex-col flex-left mb-2'>
                    <label htmlFor='description'>Blog Content (For images: Upload first, copy the link, then use `![alt text](link)`)</label>
                    <MarkdownEditor
                        value={description}
